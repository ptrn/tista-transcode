...
class TheoraTranscoder(Transcoder):
    ...
    def GetCMD(self, inPath, outPath, options={}):
        ret = [self.execPath, '-o', outPath]
        if 'width' in options:
          ret.append('-x')
          ret.append(options['width'])
        if 'heigth' in options:
          ret.append('-y')
          ret.append(options['heigth'])
        if 'vbitrate' in options:
          ret.append('--two-pass')
          ret.append('-V')
          ret.append(options['vbitrate'])
        if 'abitrate' in options:
          ret.append('-A')
          ret.append(options['abitrate'])
#        if 'framerate' in options:
#          ret.append('-F')
#          ret.append(options['framerate'])
        ret.append(inPath)
        return ret 
        """
...
class Handbrake(Transcoder):
    ...
    def GetCMD(self, inPath, outPath, options={}):
        return (self.execPath, '-i', inPath,'-o', outPath, 
            ...
            '-E',  self.correct.get(options.get('acodec','aac'),options.get('acodec','faac')),
...
class Ffmpeg(Transcoder):
    ...
    def __init__(self, baseURL):
        Transcoder.__init__(self, baseURL)
        self.execPath ="ffmpeg"
        self.recipe = "ffmpeg"

    def CanDo(self, options):
        if self.execPath == None: return False
        if options.get('format','') == 'ts': return True 
        return False
    ...
    def GetCMD(self, inPath, outPath, options={}):
        ...
        r = "%f" % (fps,)
        if fps < 25: r = '25'
        return (self.execPath, '-i', inPath,
            '-f', 'mpegts', 
#            '-ar', '44100',
#            '-vb',  str(options.get('vbitrate')*1000),
#            '-ab',  '64000',
#            '-s', "%sx%s" % (options.get('width'),options.get('height')), 
            '-r', r,
            outPath)

    def Glue(self,root,sources,target, fps=25): 
        ttc.pretext.glueTSPretext(root,sources, target, fps)
...
def ChangeArgs(job,w,h,vb,ab,r):
    args = dict(job)
    args['format'] = 'ts' 
    args['vcodec'] = 'x264' 
    args['acodec'] = 'faac'
...
def GetInfo(path):
    ...
    if u"bitrate" not in finfo[u"audio"][0]: finfo[u"audio"][0][u"bitrate"] = finfo[u"bitrate"]
    return finfo
...
# Get properties of src 
        ...
        defFrameRate = info[u"video"][0][u"framerate"]   #.partition(':')[0]
# Get wanted properties
        width  = d.get("width",defWidth)
        height = d.get("height",defHeight)
        vbitrate  = d.get("vbitrate",defVBitrate)
        abitrate  = d.get("abitrate",defABitrate)
      ...
      try:
        ...
        if imgPath:
          mp4trans     = Ffmpeg(baseURL)
          mp4args      = ChangeArgs(d,defWidth,defHeight,defVBitrate,defABitrate,defFrameRate)
          d['framerate'] = defFrameRate
          soundPath    = "silence.mp3"
          termPath     = os.path.join(root, "intermediate.ts")
          textPath     = os.path.join(root, "pretext")
          blankPath    = os.path.join(root, "blank")
          outTextPath  = os.path.join(root, "pretext.ts")
          outBlankPath = os.path.join(root, "blank.ts")
          resPath      = os.path.join(root, "encapsulated")
          ttc.pretext.makeTSPretext(root, float(imgDur),imgPath, soundPath, textPath, int(defVBitrate), int(defABitrate), int(defWidth), int(defHeight), defFrameRate)
          ttc.pretext.makeTSPretext(root, float(sepDur), False, soundPath, blankPath, int(defVBitrate), int(defABitrate), int(defWidth), int(defHeight), defFrameRate)
          mp4trans.Transcode(root, jobID, inPath, termPath, mp4args)
          mp4trans.Transcode(root, jobID, textPath, outTextPath, mp4args)
          mp4trans.Transcode(root, jobID, blankPath, outBlankPath, mp4args)
          mp4trans.Glue(root,[outTextPath, outBlankPath, termPath, outBlankPath, outTextPath], resPath, defFrameRate)
        else:
          resPath = inPath 
        transcoder.Transcode(root, jobID, resPath, outPath, d)
      except Exception as e:
        arg = "%s" % urllib.urlencode([("id",jobID)])
        urllib2.urlopen("%sjobs/error?%s" % (baseURL,arg))
        ...
        try:
            transcoder.Upload(root, outPath, dstURI, jobID, "video/%s" % (format,))
        except Exception as e:
            arg = "%s" % urllib.urlencode([("id",jobID)])
            urllib2.urlopen("%sjobs/error?%s" % (baseURL,arg))
